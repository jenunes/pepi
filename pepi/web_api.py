...
def analyze_queries_route(
    ...
    try:
        ...
        health_by_key = {
            (q.namespace, q.operation, q.pattern): q for q in awr.queries
        }
        ...
        for (ns, op, pattern), stats in query_stats.items():
            ...
            enriched = health_by_key.get((ns, op, pattern))
            if enriched is not None:
                row.update(
                    {
                        "health_score": enriched.health_score,
                        "health_severity": enriched.health_severity,
                        "scan_ratio": enriched.scan_ratio,
                        "key_efficiency": enriched.key_efficiency,
                        "findings_count": enriched.findings_count,
                        "in_memory_sort_pct": enriched.in_memory_sort_pct,
                        "disk_usage_pct": enriched.disk_usage_pct,
                        "yield_rate": enriched.yield_rate,
        ...
        return AnalysisResult(
            ...
            data={
                ...
                "summary": awr_data["summary"],
                "findings": awr_data["findings"],
                **collscan_trends,
            },
        )
    except Exception as e:
