...
class CustomCommand(click.Command):
    ...
    def show_contextual_help(self, option, formatter):
        ...
        if option == "--connections":
            with formatter.section("Connection Analysis"):
                ...
                formatter.write_text(
                    "  pepi --fetch logfile --connections --compare ip1 --compare ip2"
                )
                formatter.write_text(
                    "  pepi --fetch logfile --connections --stats --sort-by opened "
                    "--compare ip1 --compare ip2"
                )
            ...
            with formatter.section("Query Analysis"):
                ...
                formatter.write_text(
                    "  --sort-by        Sort by: count | min | max | 95% | sum | mean"
                )
                formatter.write_text("  --report-full-patterns  Write complete patterns to file")
                formatter.write_text(
                    "  --namespace      Filter by namespace (e.g., 'database.collection')"
                )
                formatter.write_text(
                    "  --operation      Filter by operation type (e.g., 'find', 'insert', 'update')"
                )
                formatter.write_text(
                    "  --report-histogram  Show execution time distribution histogram"
                )
                formatter.write_text("")
                formatter.write_text("Examples:")
                ...
                formatter.write_text(
                    "  pepi --fetch logfile --queries --report-full-patterns report.txt"
                )
            ...
            with formatter.section("Log File Trimming"):
                ...
                formatter.write_text(
                    "  --from DATE      Start date/time (format: DD/MM/YYYY HH:MM:SS:MS)"
                )
                formatter.write_text(
                    "  --until DATE     End date/time (format: DD/MM/YYYY HH:MM:SS:MS)"
                )
                formatter.write_text("")
                ...
                formatter.write_text(
                    "  - If only date provided: assumes whole day (00:00:00 to 23:59:59)"
                )
                formatter.write_text(
                    "  - If --from and --until are equal: full period of that timeframe"
                )
                ...
                formatter.write_text(
                    "  pepi --fetch logfile --trim --from '25/12/2023' --until '26/12/2023'"
                )
                formatter.write_text(
                    "  pepi --fetch logfile --trim --from '25/12/2023 14:00' "
                    "--until '25/12/2023 16:00'"
                )
                formatter.write_text(
                    "  pepi --fetch logfile --trim --from '25/12/2023 14:30:45' "
                    "--until '25/12/2023 14:30:45'"
                )
            ...
            with formatter.section("Web Interface"):
                ...
                formatter.write_text(
                    "  pepi --fetch logfile --web-ui     # Launch with pre-loaded file"
                )
                formatter.write_text("")
                formatter.write_text(
                    "The web interface will open in your default browser at http://localhost:8000"
                )
    ...
    def show_full_help(self, formatter):
        ...
        with formatter.section("Description"):
            formatter.write_text(
                "pepi: MongoDB log analysis tool for extracting insights from MongoDB log files."
            )
        ...
        with formatter.section("Log File Operations"):
            formatter.write_text(
                "--trim              Trim log file by date/time range (use with --from and --until)"
            )
            formatter.write_text(
                "--web-ui            Launch web interface with specified log file pre-loaded"
            )
            formatter.write_text("--clear-cache       Clear all cached data and re-parse files")

        with formatter.section("Default Behavior"):
            formatter.write_text(
                "When no analysis mode is specified, shows MongoDB log summary and "
...
def launch_web_ui(logfile: Optional[str] = None, sample_percentage: int = 100) -> None:
    ...
    try:
        process = subprocess.Popen([sys.executable, "-m", "pepi.web_api"], env=env, text=True)
...
@click.option(
    "--fetch", "-f", "logfile", type=click.Path(exists=True), help="MongoDB log file to analyze."
)
@click.option("--rs-conf", is_flag=True, help="Print replica set configuration(s) from the log.")
@click.option(
    "--rs-state", is_flag=True, help="Print replica set node status and state transitions."
)
@click.option("--connections", is_flag=True, help="Print connection information and statistics.")
@click.option(
    "--stats", is_flag=True, help="Include connection duration statistics (use with --connections)."
)
@click.option(
    "--sort-by",
    type=click.Choice(["opened", "closed", "count", "min", "max", "95%", "sum", "mean"]),
    help="Sort by specified metric (use with --connections or --queries).",
)
@click.option(
    "--compare", multiple=True, help="Compare 2-3 specific hostnames/IPs (use with --connections)."
)
@click.option(
    "--clients", is_flag=True, help="Print client/driver information and authentication details."
)
@click.option(
    "--queries", is_flag=True, help="Print query pattern statistics and performance analysis."
)
@click.option(
    "--report-full-patterns",
    type=click.Path(),
    help="Write complete query patterns to file (requires output file path).",
)
@click.option(
    "--namespace", type=str, help='Filter queries by namespace (e.g., "database.collection").'
)
@click.option(
    "--operation",
    type=str,
    help=(
        'Filter queries by operation type (e.g., "find", "insert", "update", '
        '"delete", "aggregate").'
    ),
)
@click.option(
    "--report-histogram", is_flag=True, help="Show histogram of execution time distribution."
)
@click.option("--clear-cache", is_flag=True, help="Clear all cached data and re-parse files.")
@click.option(
    "--trim", is_flag=True, help="Trim log file by date/time range (use with --from and --until)."
)
@click.option(
    "--from",
    "from_date",
...
@click.option(
    "--web-ui", is_flag=True, help="Launch web interface with the specified log file pre-loaded."
)
@click.option("--version", is_flag=True, help="Show version information and exit.")
@click.option("--upgrade", is_flag=True, help="Check for updates and upgrade Pepi.")
@click.option(
    "--sample",
    ...
    if rs_conf:
        ...
        if sample == 100:
            try:
                ...
                if line_count > 50000:
                    click.echo(
                        f"⚠️  Warning: Processing 100% of a large file "
                        f"({line_count:,} lines) may be slow.",
                        err=True,
                    )
                    click.echo("   Consider using --sample 50 for faster analysis.", err=True)
                    click.echo()
            except Exception:
                pass

        connections_data, total_opened, total_closed = parse_connections(
            logfile, sample_percentage=sample
        )
        ...
        if sort_by:
            ...
            if sort_by == "opened":
                sorted_ips = sorted(
                    connections_data.keys(),
                    key=lambda ip: connections_data[ip]["opened"],
                    reverse=True,
                )
            elif sort_by == "closed":
                sorted_ips = sorted(
                    connections_data.keys(),
                    key=lambda ip: connections_data[ip]["closed"],
                    reverse=True,
                )
        else:
        ...
        if compare:
            if len(compare) < 2:
                ...
                click.echo(
                    "Error: --compare accepts maximum 3 hostnames/IPs. "
        ...
        for ip in sorted_ips:
            ...
            if stats and ip in ip_stats:
                ...
                click.echo(
                    f"{ip.ljust(max_ip_len)} | "
                    f"opened:{str(conn_info['opened']).ljust(5)} | "
                    f"closed:{str(conn_info['closed']).ljust(5)} | "
                    f"dur-avg:{stats_info['avg']:.2f}s | "
                    f"dur-min:{stats_info['min']:.2f}s | "
                    f"dur-max:{stats_info['max']:.2f}s"
                )
            else:
        ...
        if report_full_patterns:
            with open(report_full_patterns, "w") as f:
                ...
                if sort_by:
                    if sort_by == "count":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["count"], reverse=True
                        )
                    elif sort_by == "min":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["min"], reverse=True
                        )
                    elif sort_by == "max":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["max"], reverse=True
                        )
                    elif sort_by == "95%":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["percentile_95"], reverse=True
                        )
                    elif sort_by == "sum":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["sum"], reverse=True
                        )
                    elif sort_by == "mean":
                        sorted_queries = sorted(
                            query_stats.items(), key=lambda x: x[1]["mean"], reverse=True
                        )
                    else:
                ...
                if sorted_queries:
                    ...
                    max_percentile_len = max(
                        len(f"{stats['percentile_95']:.1f}") for stats in query_stats.values()
                    )
                    max_sum_len = max(len(f"{stats['sum']:.1f}") for stats in query_stats.values())
                    max_mean_len = max(
                        len(f"{stats['mean']:.1f}") for stats in query_stats.values()
                    )
                    max_index_len = max(
                        len(", ".join(sorted(stats["indexes"]))) for stats in query_stats.values()
                    )
                ...
                header_fmt = (
                    f"{{:<{max_namespace_len}}} | {{:<{max_operation_len}}} | "
                    f"{{:<{max_pattern_len}}} | {{:>{max_count_len}}} | {{:>{max_min_len}}} | "
                    f"{{:>{max_max_len}}} | {{:>{max_percentile_len}}} | {{:>{max_sum_len}}} | "
                    f"{{:>{max_mean_len}}} | {{:<8}} | {{:<{max_index_len}}}"
                )
                f.write(
                    header_fmt.format(
                        "Namespace",
                ...
                for (namespace, operation, pattern), stats_info in sorted_queries:
                    indexes_display = (
                        ", ".join(sorted(stats_info["indexes"])) if stats_info["indexes"] else "N/A"
                    )
                    row_fmt = (
                        f"{{:<{max_namespace_len}}} | {{:<{max_operation_len}}} | "
                        f"{{:<{max_pattern_len}}} | {{:>{max_count_len}}} | {{:>{max_min_len}}} | "
        ...
        if sorted_queries:
            ...
            max_operation_len = max(len(str(key[1])) for key in query_stats.keys())
            max_pattern_len = max(
                len(pattern[:150] + "..." if len(pattern) > 150 else pattern)
                for (_, _, pattern) in query_stats.keys()
            )
            max_count_len = max(len(str(stats["count"])) for stats in query_stats.values())
            max_min_len = max(len(f"{stats['min']:.1f}") for stats in query_stats.values())
            max_max_len = max(len(f"{stats['max']:.1f}") for stats in query_stats.values())
